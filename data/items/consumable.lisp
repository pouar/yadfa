...
(defclass bottle-of-milk (consumable) ()
    (:default-initargs
        ...
        :use-script '(lambda (item user)
                         ...
                         (if (> (health-of user) 0)
                             (progn
                                 (format t "~a regained health~%" (name-of user))
                                 (incf (health-of user) 20))
                             (progn
                                 (format t "You make the unconscious ~a suckle on the ~a like a sleeping infant~%"
                                     (name-of user)
                                     (name-of item)))))))
    ...
    (:default-initargs
        ...
        :cant-use-predicate '(lambda (item user &rest keys &key target action &allow-other-keys)
                                 ...
                                 (when (<= (health-of target) 0)
                                     (format t "Does ~a look conscious enough to use that?~%"
                                         (name-of target))))
        :use-script '(lambda (item user)
                         (declare (ignore item))
    ...
    (:default-initargs
        ...
        :cant-use-predicate '(lambda (item user &rest keys &key target action &allow-other-keys)
                                 ...
                                 (when (>= (health-of target) (calculate-stat target :health))
                                     (format t "~a's health is already full~%"
                                         (name-of target))))
        :use-script '(lambda (item user)
                         (declare (ignore item))
                         (incf (health-of user) 20))))
(defclass revive (consumable) ()
    (:default-initargs
        ...
        :cant-use-predicate '(lambda (item user &rest keys &key target action &allow-other-keys)
                                 (declare (ignorable item user keys target action))
                                 (when (> (health-of target) 0)
                                     (format t "Does ~a look unconscious to you?~%"
                                         (name-of target))))
        :use-script '(lambda (item user)
                         (declare (ignore item))
                         (incf (health-of user) 20))))
