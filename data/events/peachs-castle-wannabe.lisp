...
(defevent enter-thwomp-area-1
  :lambda '(lambda (self)
            ...
            (out "Thwomp: You will never get past me, because once you stand under me, you're a gonner.")))
(defevent win-thwomp-area-1
  :lambda '(lambda (self)
            (declare (ignore self))
            (out "Thwomp: HEY!!! NO FAIR!!!! YOU KNOW I CAN ONLY MOVE UP AND DOWN!!!!!!!")
            (out "You collect the star")
            (push (make-instance 'yadfa-items:shine-star) (inventory-of (player-of *game*)))))
(defevent enter-pokemon-area-1
  :lambda '(lambda (self)
            (declare (ignore self))
            (out "You find a Pokemon trainer")
            (out "Pokemon trainer: OK, Bowser has paid me to guard this star, so I'm going to fight you with all my might, with my level 2 Magikarp")
