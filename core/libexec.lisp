  ...
  (if (gethash (list :map-pattern path designs) *pattern-cache*)
      ...
      (setf (gethash (list :map-pattern path designs) *pattern-cache*)
            (clim:make-pattern-from-bitmap-file
             ...
             :format :xpm
             :designs designs))))
(defunassert (print-map (position))
    (position list)
...
(defunassert (total-thickness (clothing))
  ...
  (iter (for i in (filter-items clothing 'closed-bottoms))
    ...
    (finally (return j))))
(defun fast-thickness (list item count)
  #+sbcl (declare (type list list)
                  (type clothing item)
                  (type number count))
  (cond ((eq (car list) item) count)
        ((typep (car list) 'closed-bottoms)
         (fast-thickness (cdr list) item (+ count (get-diaper-expansion (car list)))))
        (t (fast-thickness (cdr list) item count))))
