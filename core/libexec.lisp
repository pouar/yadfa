...
(defun set-new-battle (enemies &rest keys &key team-npcs win-events enter-battle-text continuable)
  ...
  (setf *battle*
        (apply #'make-instance 'battle
               ...
               :team-npcs (iter (for j in team-npcs)
                            (collect (apply #'make-instance (car j) (eval (cdr j)))))
               keys))
...
(defun intro-function ()
  ...
  (let* ((default (make-instance 'player))
         ...
         name male species clothes bladder fill-rate wings skin tail tail-type bio)
    ...
    (setf (player-of *game*) (make-instance 'player
                                            ...
                                            :bowels/fill-rate (getf `(:normal ,(bowels/fill-rate-of default)
                                                                      :fast ,(* (bowels/fill-rate-of default) 2)
                                                                      :faster ,(* (bowels/fill-rate-of default) 4))
                                                                    fill-rate)
                                            :bladder/fill-rate (getf `(:normal ,(bladder/fill-rate-of default)
                                                                       :fast ,(* (bladder/fill-rate-of default) 2)
                                                                       :faster ,(* (bladder/fill-rate-of default) 4))
                                                                     fill-rate)
