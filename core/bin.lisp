      ...
      (cond ((not user)
            ...
            ((typep user 'integer)
             (let ((selected-ally (nth user (allies-of *game*))))
               (when (< allies-length user)
                 (format t "You only have ~d allies~%~%" allies-length)
                 (return-from yadfa-bin:lst))
               (format t "Number of items listed: ~a~%" (iter (with j = 0)
                                                          (for i in (wear-of selected-ally))
                                                          (when (typep i wear) (incf j))
                                                          (finally (return j))))
               (format t "~a:~%~%" (name-of selected-ally))
               (format t "~7a~40a~6a~8a~6a~8a~%" "Index" "Name" "Wet" "Wetcap" "Mess" "Messcap")
               (let ((j 0)) (iter (for i in (wear-of selected-ally))
                              (when (typep i wear)
                                (format t "~7a~40a~6a~8a~6a~8a~%" j
                                        (name-of i)
                                        (if (typep i 'closed-bottoms) (sogginess-of i) nil)
    ...
    (when moves
      (cond ((typep user 'number)
             (let ((selected-ally (nth user (allies-of *game*))))
               ...
               (iter (for i in (moves-of selected-ally))
  ...
  (let ((selected-item (cond ((typep item 'unsigned-byte)
    ...
    (cond ((not item)
          ...
          ((and enemy-target (< enemies-length enemy-target))
           (format t "You only have ~d allies~%" enemies-length)
           (return-from yadfa-battle:use-item))
