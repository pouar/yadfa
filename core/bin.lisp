...
(defun yadfa-bin:toggle-onesie (&key wear user)
  ...
  (cond (user
         (cond ((>= user (list-length (allies-of *game*)))
               ...
               (t (iter (for i in (wear-of (nth user (allies-of *game*))))
                    (with j = 1)
  ...
  (iter (for direction in directions)
    (let* ((new-position (get-path-end (get-destination direction (position-of (player-of *game*))) (position-of (player-of *game*)) direction))
      ...
      (when (iter (for i in (cons (player-of *game*) (allies-of *game*)))
              (when (or (and (< (list-length (filter-items (wear-of i) (car (must-wear-of (get-zone new-position))))) 1)
                             (not (funcall (coerce (cdr (must-wear-of (get-zone new-position))) 'function) i)))
                        (and (> (list-length (filter-items (wear-of i) (car (must-not-wear-of (get-zone new-position))))) 0)
                             (not (funcall (coerce (cdr (must-not-wear-of (get-zone new-position))) 'function) i))))
                (leave t)))
        (return-from yadfa-world:move))
      (when (or (and (not (eq (lockedp (get-zone new-position)) :nil))
                     (not (member (lockedp (get-zone new-position)) (inventory-of (player-of *game*))
                                  :test (lambda (item key)
                                          (typep key item)))))
                (and (not (eq (getf (getf (direction-attributes-of (get-zone (position-of (player-of *game*)))) direction) :locked :nil) :nil))
                     (not (member (getf (getf (direction-attributes-of (get-zone (position-of (player-of *game*)))) direction) :locked :nil)
      ...
      (when (or (and (not (eq (lockedp (get-zone new-position)) :nil)) (member (lockedp (get-zone new-position)) (inventory-of (player-of *game*))
                                                                               :test (lambda (item key)
                                                                                       (typep key item))))
                (and (not (eq (getf (getf (direction-attributes-of (get-zone (position-of (player-of *game*)))) direction) :locked :nil) :nil))
                     (member (getf (getf (direction-attributes-of (get-zone (position-of (player-of *game*)))) direction) :locked :nil)
                             (inventory-of (player-of *game*))
                             :test (lambda (item key)
                                     (typep key item)))))
...
(defun yadfa-bin:lst (&key inventory inventory-group props wear user directions moves position map descriptions describe-zone)
  ...
  #+sbcl (declare (type (or unsigned-byte boolean) user) (type (or boolean integer) map) (type type-specifier inventory))
  ...
  (when wear
    (cond ((not user)
           ...
           (iter (for k in (allies-of *game*))
             ...
             (format t "~7a~40a~6a~8a~6a~8a~%" "Index" "Name" "Wet" "Wetcap" "Mess" "Messcap")
             (let ((j 0)) (iter (for i in (wear-of k))
                            (when (typep i wear)
                              (format t "~7a~40a~6a~8a~6a~8a~%" j
                                      (name-of i)
                                      (if (typep i 'closed-bottoms) (sogginess-of i) nil)
          ...
          ((typep user 'integer)
           ...
           (format t "~7a~40a~6a~8a~6a~8a~%" "Index" "Name" "Wet" "Wetcap" "Mess" "Messcap")
           (let ((j 0)) (iter (for i in (wear-of (nth user (allies-of *game*))))
                          (when (typep i wear)
                            (format t "~7a~40a~6a~8a~6a~8a~%" j
                                    (name-of i)
                                    (if (typep i 'closed-bottoms) (sogginess-of i) nil)
          ...
          (t
           ...
           (let ((j 0))
             ...
             (write-char #\Newline)))))
  (when moves
    (cond ((typep user 'number)
           (format t "~a:~%~%" (name-of (nth user (allies-of *game*))))
           (format t "~30a~20a~40a~%" "Symbol" "Name" "Description")
           (iter (for i in (moves-of (nth user (allies-of *game*))))
             (when i (format t "~30s~20a~40a~%" (class-name (class-of i)) (name-of i) (description-of i)))))
          ((not user)
           (format t "~a:~%~%" (name-of (player-of *game*)))
           (format t "~30a~20a~40a~%" "Symbol" "Name" "Description")
           (iter (for i in (moves-of (player-of *game*)))
             (when i
               (format t "~30s~20a~40a~%" (class-name (class-of i)) (name-of i) (description-of i))))
           (write-char #\Newline)
           (iter (for k in (allies-of *game*))
             (format t "~a:~%~%" (name-of k))
             (format t "~30a~20a~40a~%" "Symbol" "Name" "Description")
             (iter (for i in (moves-of k))
               (when i
                 (format t "~30s~20a~40a~%" (class-name (class-of i)) (name-of i) (description-of i))))
             (write-char #\Newline)))
          (t
...
(defun yadfa-bin:get-stats (&key inventory wear prop item attack ally wield)
  ...
  #+sbcl (declare (type (or null unsigned-byte) ally wear inventory) (type (or null keyword) prop))
...
(defun yadfa-world:interact (prop &rest keys &key list take action describe-action describe &allow-other-keys)
  ...
  #+sbcl (declare (type (or keyword null) action describe-action) (type symbol prop) (type list describe) (type (or null keyword list) take))
...
(defun yadfa-bin:wear (&key (inventory 0) (wear 0) user)
  ...
  #+sbcl (declare (type (or null unsigned-byte) user) (type unsigned-byte wear) (type (or type-specifier unsigned-byte) wear inventory))
  ...
  (let* ((selected-user (if user
    ...
    (cond ((not item)
          ...
          ((not (typep item 'clothing))
           ...
           (return-from yadfa-bin:wear))
          ((and (typep item (car (must-not-wear*-of (get-zone (position-of (player-of *game*))))))
                (not (funcall (coerce (cdr (must-not-wear*-of (get-zone (position-of (player-of *game*))))) 'function) selected-user)))
           (format t "~a isn't allowed to wear those here.~%" (name-of selected-user))
           (return-from yadfa-bin:wear))
          ((and (> wear 0) (iter (for i in (butlast (wear-of selected-user) (- (list-length (wear-of selected-user)) wear)))
...
(defun yadfa-bin:unwear (&key (inventory 0) (wear 0) user)
  ...
  #+sbcl (declare (type (or unsigned-byte null) user) (type (or (and symbol (not keyword)) list unsigned-byte) wear) (type unsigned-byte inventory))
  (check-type user (or unsigned-byte null))
  ...
  (let* ((selected-user (if user
    ...
    (cond ((not item)
          ...
          ((> inventory (list-length (inventory-of (player-of *game*))))
           (format t "`:INVENTORY ~d' doesn't refer to a valid position as it can't go past the items you currently have in your inventory which is currently ~d~%" inventory (list-length (inventory-of (player-of *game*))))
           (return-from yadfa-bin:unwear))
          ((and
           ...
           (format t "Letting ~a go without padding is a really bad idea. Don't do it.~%"
                   (name-of selected-user))
           (return-from yadfa-bin:unwear))
          ((and (typep item (car (must-wear*-of (get-zone (position-of (player-of *game*))))))
                (<= (list-length (filter-items (wear-of selected-user) (car (must-wear*-of (get-zone (position-of (player-of *game*))))))) 1)
                (not (funcall (coerce (cdr (must-wear*-of (get-zone (position-of (player-of *game*))))) 'function) selected-user)))
           (return-from yadfa-bin:unwear))
    ...
    (when *battle*
      (format t "The ~a you're battling stops and waits for you to take off your ~a because Pouar never prevented this function from being called in battle~%"
              (if (> (list-length (enemies-of *battle*)) 1)
                  "enemies"
                  "enemy")
              (name-of item)))
...
(defun yadfa-bin:change (&key (inventory 0) (wear 0) user)
  ...
  #+sbcl (declare (type (or null unsigned-byte) user) (type (or type-specifier unsigned-byte) inventory wear))
  (check-type user (or null unsigned-byte))
  (check-type inventory (or type-specifier unsigned-byte))
  (check-type wear (or type-specifier unsigned-byte))
  (let* ((selected-user (if user
    ...
    (cond ((not inventory)
          ...
          ((and
           ...
           (return-from yadfa-bin:change))
          ((or (and (not (typep inventory (car (must-wear*-of (get-zone (position-of (player-of *game*)))))))
                    (typep wear (car (must-wear*-of (get-zone (position-of (player-of *game*))))))
                    (<= (list-length (filter-items (wear-of selected-user) (car (must-wear*-of (get-zone (position-of (player-of *game*))))))) 1)
                    (not (funcall (coerce (cdr (must-wear*-of (get-zone (position-of (player-of *game*))))) 'function) selected-user)))
               (and (typep inventory (car (must-not-wear*-of (get-zone (position-of (player-of *game*))))))
                    (not (funcall (coerce (cdr (must-not-wear*-of (get-zone (position-of (player-of *game*))))) 'function) selected-user))))
           (return-from yadfa-bin:change))
          ((and
...
(defun yadfa-battle:fight (attack &key target friendly-target)
  ...
  #+sbcl (declare (type (or null unsigned-byte) target) (type (or symbol boolean) attack))
...
(defun yadfa-battle:stats (&key user enemy)
  ...
  #+sbcl (declare (type (or unsigned-byte null) user enemy))
  (check-type user (or unsigned-byte null))
...
(defun yadfa-world:go-potty (&key prop wet mess pull-pants-down user)
  ...
  #+sbcl (declare (type (or null number) user) (type (or null keyword) prop) (type (or boolean number) wet mess))
...
(defun yadfa-world:use-item (item &rest keys &key user action &allow-other-keys)
  ...
  #+sbcl (declare (type (or unsigned-byte (and symbol (not keyword)) list) item) (type (or null keyword) action))
...
(defun yadfa-battle:use-item (item &key target enemy-target)
  ...
  #+sbcl (declare (type (or unsigned-byte (and symbol (not keyword)) list) item) (type (or null unsigned-byte) target enemy-target))
  (check-type item (or unsigned-byte (and symbol (not keyword)) list))
  (check-type target (or null unsigned-byte))
...
(defun yadfa-battle:reload (ammo-type)
  #+sbcl (declare (type (and (or list (and symbol (not keyword))) (not null)) ammo-type))
  ...
  (unless (and (ammo-type-of (wield-of (first (turn-queue-of *battle*)))) (> (ammo-capacity-of (wield-of (first (turn-queue-of *battle*)))) 0))
    ...
    (return-from yadfa-battle:reload))
  (when
      (>= (list-length (ammo-of (wield-of (first (turn-queue-of *battle*))))) (ammo-capacity-of (wield-of (first (turn-queue-of *battle*)))))
...
(defun yadfa-world:reload (ammo-type &optional user)
  #+sbcl (declare (type (and (or list (and symbol (not keyword))) (not null)) ammo-type) (type (or unsigned-byte null) user))
  (check-type ammo-type (and (or list (and symbol (not keyword))) (not null)))
  (check-type user (or unsigned-byte null))
  (let ((user (if user
                  (nth user (allies-of *game*))
    ...
    (unless (and (ammo-type-of (wield-of user)) (> (ammo-capacity-of (wield-of user)) 0))
      ...
      (return-from yadfa-world:reload))
    (when (>= (list-length (ammo-of (wield-of user))) (ammo-capacity-of (wield-of user)))
    ...
    (iter (with count = 0)
      ...
      (when (or (>= (list-length (ammo-of (wield-of user))) (ammo-capacity-of (wield-of user)))
...
(defun yadfa-bin:wield (&key user inventory)
  ...
  #+sbcl (declare (type (or unsigned-byte null) user) (type (or (and symbol (not keyword)) list class null unsigned-byte) inventory))
  (check-type user (or unsigned-byte null))
...
(defun yadfa-world:swap-team-member (team-index-1 team-index-2)
  ...
  (cond ((or (>= team-index-1 (list-length (team-of *game*))) (>= team-index-2 (list-length (team-of *game*))))
         (format t "You only have ~d members in your team~%" (list-length (team-of *game*)))
  ...
  (let ((selected-user (if user (nth user (allies-of *game*)) (player-of *game*))))
    (cond ((not selected-user)
          ...
          ((not (< wear (list-length (wear-of selected-user))))
           (format t "~a is only wearing ~d items~%" (name-of selected-user) (list-length (wear-of selected-user))))
          ((not (< key (list-length (inventory-of (player-of *game*)))))
           (format t "You only have ~d items in your inventory~%" (list-length (inventory-of (player-of *game*)))))
          ...
          ((lockedp (nth wear (wear-of selected-user)))
           (format t "~a's ~a is now unlocked~%" (name-of selected-user) (name-of (nth wear (wear-of selected-user))))
